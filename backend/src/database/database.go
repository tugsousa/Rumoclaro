...
func RunMigrations(databasePath string) {
	...
	migrationsSourceURL := fmt.Sprintf("file://%s", config.Cfg.MigrationsPath)
	logger.L.Info("Using migrations source URL", "url", migrationsSourceURL)
	// The path is relative to the backend executable's location
	m, err := migrate.NewWithDatabaseInstance(
		migrationsSourceURL, // <-- USE THE DYNAMIC URL
		databasePath,
		driver,
	)
	if err != nil {
