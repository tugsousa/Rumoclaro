...
const deleteTransactionsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // --- FIX: Invalidate all other relevant queries ---
      queryClient.invalidateQueries({ queryKey: ['stockSales', token] });
      queryClient.invalidateQueries({ queryKey: ['optionSales', token] });
      queryClient.invalidateQueries({ queryKey: ['dividendSummary', token] });
      queryClient.invalidateQueries({ queryKey: ['dividendTransactions', token] });
      queryClient.invalidateQueries({ queryKey: ['stockHoldingsByYear', token] });
      queryClient.invalidateQueries({ queryKey: ['optionHoldings', token] });
      queryClient.invalidateQueries({ queryKey: ['currentHoldingsValue', token] });
      // This helper will re-check if the user has any data, updating UI elements
    ...
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      ...
      {processedTransactions.length > 0 ? (
        ...
        <Typography sx={{ textAlign: 'center', mt: 4, fontStyle: 'italic', color: 'text.secondary' }}>
          {transactionsLoading ? "A carregar..." : "Sem dados disponíveis. Por favor, carregue primeiro um ficheiro de transações."}
        </Typography>
      )}
