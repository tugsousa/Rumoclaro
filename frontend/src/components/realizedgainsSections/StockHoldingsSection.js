...
const renderCurrentPriceCell = (params) => {
    ...
    if (typeof value !== 'number' || value === 0) {
        return <Tooltip title="Preço atual indisponível." placement="top"><span>N/A</span></Tooltip>;
    }
  ...
  { field: 'buy_currency', headerName: 'Moeda', width: 90 },
{
  ...
  { field: 'buy_amount_eur', headerName: 'Montante (€)', type: 'number', width: 130, align: 'right', headerAlign: 'right', valueGetter: (_, row) => Math.abs(row.buy_amount_eur || 0), valueFormatter: (value) => formatCurrency(value) },
];

// Colunas para a vista AGRUPADA (CURRENT holdings)
const groupedColumnsCurrent = [
  { field: 'product_name', headerName: 'Produto', flex: 1, minWidth: 160 },
  { field: 'isin', headerName: 'ISIN', width: 130 },
  { field: 'quantity', headerName: 'Qtd', type: 'number', width: 40, align: 'right', headerAlign: 'right' },
    ...
    valueGetter: (value, row) => {
        ...
        return row.total_cost_basis_eur / row.quantity;
    },
